...
# JavaScript equivalent: [...Array(10)].map((_, i) => (i+1)**2)
squares = ???


# Create a list of even numbers from 1 to 20
# JavaScript equivalent: [...Array(20)].map((_, i) => i+1).filter(n => n % 2 === 0)
evens = ???
...
# Get only negative numbers as absolute values
abs_negatives = ???

# Get numbers greater than 0 and less than 5
small_positives = ???


# Exercise 3.3: Comprehension with transformation and filter

# Get squares of even numbers from 1 to 10 only
even_squares = ???
...
# Create a 3x3 matrix filled with zeros (nested comprehension)
zeros_matrix = ???


# Create first 3 rows of a multiplication table (1-5 columns)
# Row 1: [1*1, 1*2, 1*3, 1*4, 1*5], Row 2: [2*1, 2*2, ...], etc.
mult_rows = ???


# Exercise 3.5: Dictionary comprehensions

# Create dict mapping numbers 1-5 to their squares
num_to_square = ???
...
# Get unique first letters from words (use a set comprehension)
...
# Create a generator of squares for 1-5 (use parentheses, not brackets)
square_gen = ???

# Sum of squares from 1 to 10 using a generator (pass generator directly to sum())
sum_of_squares = sum(???)


# Exercise 3.8: Comprehension with multiple conditions

# Get numbers divisible by both 2 and 3 from range(1, 31)
divisible_by_6 = ???
...
# Get (word, length) tuples for words where length > 3
# Use the walrus operator := to compute length once and reuse it
